...
def get_recommend(person, top_N):
    ...
    # 本人がまだ見たことが無い映画の集合を取得
        ...
        for item in set_new_movie:
            ...
            simSums[item] += sim
            # print('simSums[item]',simSums[item])
...
def main():
    ...
    print(get_similairty('Lisa Rose','Jack Matthews'))
    print(get_similairty('Lisa Rose','Michael Phillips'))
    print(get_similairty('Lisa Rose','Claudia Puig'))
    print(get_similairty('Lisa Rose','Mick LaSalle'))
    print(get_similairty('Lisa Rose','Jack Matthews'))
    print(get_similairty('Lisa Rose','Toby'))
    # Jack Matthews
    # print()
    ranking = get_recommend('Toby',2)
    print(ranking)
